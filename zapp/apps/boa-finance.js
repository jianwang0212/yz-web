...
const SESSION_PAYLOAD_CACHE_KEY = "boaFinance.decryptedPayload.v1";
...
async function unlockData(password) {
  ...
  try {
    ...
    const payload = cachedPayload || (await decryptPackage(encryptedPackage, password));
    if (!cachedPayload) {
      writeCachedPayload(encryptedPackage, payload);
    }
    ...
    els.unlockStatus.textContent = cachedPayload
      ? `已从 Zapp Store 会话缓存打开 ${transactions.length} 笔交易。`
      : `已解锁 ${transactions.length} 笔交易。`;
...
function requireStoreUnlock() {
  ...
  els.unlockStatus.textContent = "请先在 Zapp Store 用 Face ID 打开一次；BOA Finance 会直接继承 Store 会话。";
}
...
function renderStats() {
  if (!isUnlocked) {
    ...
    els.totalMeta.textContent = "等待 Store 会话";
...
function renderOverview() {
  if (!isUnlocked) {
    els.monthCards.innerHTML = `<div class="empty">从 Zapp Store 打开后显示 BOA 总览。</div>`;
...
function renderServices() {
  if (!isUnlocked) {
    ...
    els.detectedServices.innerHTML = `<div class="empty">从 Zapp Store 打开后显示服务/订阅。</div>`;
    els.manualServices.innerHTML = `<div class="empty">从 Zapp Store 打开后显示待确认服务。</div>`;
    return;
  }
...
function renderTransactions() {
  if (!isUnlocked) {
    ...
    els.categorySummaryTable.innerHTML = `<div class="empty">从 Zapp Store 打开后显示分类汇总表。</div>`;
    els.transactionSummary.innerHTML = `<strong>Locked</strong><span>交易明细已加密</span>`;
    els.transactionList.innerHTML = `<div class="empty">从 Zapp Store 打开后显示交易数据表。</div>`;
    renderAnswerResults();
    return;
  }
...
function applyQuestion(raw) {
  ...
  if (!isUnlocked) {
    ...
    state.answer = "先从 Zapp Store 打开一次，然后我会执行这个问题。";
    state.answerRows = [];
    state.answerKind = "";
